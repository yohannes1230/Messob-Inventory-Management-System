...
export const MasterDataConsole: React.FC<MasterDataConsoleProps> = ({
  ...
  return (
    <div className="min-h-screen bg-am-gray-50 flex flex-col">
      {/* Header Bar */}
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center space-x-3">
          <img src={mesobIcon} alt="Addis Mesob" className="h-9 w-9 rounded-full object-cover shadow-sm ring-1 ring-gray-200" />
          <div>
            <h1 className="text-lg font-bold text-am-gray-900">
        ...
        <div className="flex items-center space-x-3">
          ...
          >
            <Globe className="h-3.5 w-3.5 text-am-primary-500" />
          ...
          <button
            ...
            className="flex items-center space-x-1.5 px-4 py-1.5 bg-am-primary-500 hover:bg-am-primary-600 text-white rounded-md text-xs font-semibold shadow transition-colors"
          >
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Navigation Sidebar */}
        <nav
          aria-label={isAm ? 'የዋና መረጃ ክፍሎች' : 'Master Entities'}
          className="w-64 bg-white border-r border-gray-200 p-4 space-y-1 overflow-y-auto"
        >
            ...
            return (
              <button
                ...
                className={`w-full flex items-center space-x-3 px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                  isActive
                    ? 'bg-am-primary-50 text-am-primary-700 font-semibold shadow-xs'
                    : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'
                }`}
              >
                <span className={isActive ? 'text-am-primary-500' : 'text-gray-400'}>
                  {tab.icon}
                </span>
      ...
      {isFormOpen && (
        <div
          role="dialog"
          aria-modal="true"
          aria-labelledby="record-form-modal-title"
          className="fixed inset-0 z-50 overflow-y-auto bg-black bg-opacity-50 flex items-center justify-center p-4"
        >
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 border border-gray-200">
            <h3 id="record-form-modal-title" className="text-base font-bold text-gray-900 mb-4">
              {editingRecord
